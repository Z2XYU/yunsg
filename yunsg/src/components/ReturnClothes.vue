<template>
    <div class="return-clothes">
        <div v-if="hanfuDetails.length" class="orders-container">
            <div v-for="(order, index) in hanfuDetails" :key="index" class="order-card">
                <!-- 展示汉服详情 -->
        ...
        <div v-if="showModal" class="modal-overlay">
            <div class="modal-content">
                <div class="modal-body">
                    <p>订单已成功归还！</p>
                    <p><strong>总花费：{{ totalCost }} 元</strong></p>
                    <p><strong>{{ location }}号柜门已打开。</strong></p> <!-- 显示柜门号 -->
                </div>
    ...
    methods: {
        async fetchOrders() {
            try {
                ...
                const response = await axios.get(`/lease/myOutstandingOrders/${this.uid}`);
        ...
        async getHanfuDetails(orders) {
            try {
                this.hanfuDetails = await Promise.all(orders.map(order =>
        ...
        async confirmReturn(order) {
            ...
            try {
                // 发送归还请求，并获取归还地点和总花费
                const response = await axios.post(`/lease/CompleteOrder/${siteID}`, this.leaseOrders);
